...
from timm_.data import IMAGENET_DEFAULT_MEAN, IMAGENET_DEFAULT_STD
...
class GenEfficientNet(nn.Module):
    ...
    def forward(self, x, start_block=None, end_block=None, pool=True):
        ...
        if end_block is not None and end_block == 0:
            last_guide = x # FIXME: unable to guide block0 input because teacher and student have different stem
        ...
        for stage_idx in range(start_block, len(self.blocks)):
            for layer in self.blocks[stage_idx]:
                x = layer(x)
            if stage_idx == end_block - 1:
                last_guide = x
            if stage_idx == end_block:
                return x, last_guide
